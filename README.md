# codar-ict-curriculum
Official ICT Essentials curriculum, lesson plans, and slide outlines for Codar Tech Africa.

# Codar Tech Africa – ICT Essentials Curriculum

**Instructor:** Emmanuel Francis  
**Repository Purpose:** To store, track, and share the official ICT Essentials curriculum, lesson notes, and slide outlines for Codar Tech Africa.

## Course Overview
This course covers:
- ICT Fundamentals (Hardware, Software, Networking)
- Internet & Web Technologies (Browsers, Web Dev, Cloud)
- Cybersecurity & Data Protection
- Software Development & Programming (SDLC, OOP, SQL, Git)

## Repository Structure
- `/01_curriculum_overview.pdf` – The official curriculum documents.
- `/02_lesson_notes/` – Detailed lesson notes for each week.
- `/03_slide_outlines/` – Canva slide outlines and topic breakdowns.
- `/README.md` – This file.

## Updates
This repository is updated weekly after each class to ensure version control and transparency.

**Maintained by:** Emmanuel Francis  
**Date Created:** 21st July 2026
